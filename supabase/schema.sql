...
-- ---------- DEALER PRICING ----------
alter table public.products add column if not exists dlp       numeric;
alter table public.products add column if not exists hsn_code  text;

alter table public.profiles add column if not exists discount1 numeric not null default 0;
alter table public.profiles add column if not exists discount2 numeric not null default 0;

-- Snapshot columns so historical orders are unaffected by future discount/price changes.
alter table public.order_items add column if not exists mrp       numeric;
alter table public.order_items add column if not exists dlp       numeric;
alter table public.order_items add column if not exists net_rate  numeric;
alter table public.order_items add column if not exists discount1 numeric;
alter table public.order_items add column if not exists discount2 numeric;
