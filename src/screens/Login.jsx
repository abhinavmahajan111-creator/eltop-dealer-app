        ...
        <div onClick={() => setShowFanmanModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div onClick={e => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
              <a href="/assets/fan%20man%20eltop.png" download="Eltop-Fanman.png" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 24px', borderRadius: 8, background: '#7B2D8B', color: 'white', textDecoration: 'none', fontWeight: 'bold', fontSize: 14 }}>⬇️ Download</a>
              <button onClick={async () => { if (navigator.share) { await navigator.share({ title: 'Hey I am Eltop Fanman! 🎉', text: 'Check out Eltop Fanman - brand mascot of Eltop by Embassy!', url: window.location.origin + '/store' }); } else { navigator.clipboard.writeText(window.location.origin + '/store'); alert('Link copied to clipboard!'); } }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 24px', borderRadius: 8, background: '#FF0000', color: 'white', border: 'none', cursor: 'pointer', fontWeight: 'bold', fontSize: 14 }}>🔗 Share</button>
              <button onClick={() => setShowFanmanModal(false)} style={{ padding: '10px 24px', borderRadius: 8, background: '#333', color: 'white', border: 'none', cursor: 'pointer', fontWeight: 'bold', fontSize: 14 }}>✕ Close</button>
            </div>
          </div>
        </div>
      )}
